...
export default async function ManagerDashboardPage() {
  ...
  const result = await getPendingApprovals('manager', session.branchId)
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Review and approve POP requisitions"
      />
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-[#64748B]">Awaiting Approval</p>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <Button asChild variant="outline" size="sm">
            <Link href="/manager/approvals">View All</Link>
          </Button>
        </CardHeader>
        <CardContent>
          {pending.length === 0 ? (
